...
static int kobject_add_internal(struct kobject *kobj)
{
	...
	//priv->kobj.kset = bus->p->drivers_kset; 在注册spi driver的时候
	/* join kset if set, use it as parent if we do not already have one */

	// 当platform 注册调用过来的时候，他没有父对象，但是有kset，这个kset是 /sys/devices,
	// 所以platform 就以/sys/devices/platform 这样的形式添加进来了

	// 当注册/sys/devices call过来的时候 kset->kobj.kset = NULL;  这里是空的
	if (kobj->kset) {
		if (!parent) /*kobj属于某个kset，但是该kobj没有父对象，则以kset的kobj作为父对象*/ 
					//所以当 platform调用到这里的时候，实际是将/sys/devices 作为了父对象
...
// 当注册/sys/devices call过来的时候
	//kobject: 'devices' (ef020258): kobject_add_internal: parent: '<NULL>', set: '<NULL>'
// 当注册/sys/devices/platfrom call过来的时候
	//kobject: 'platform' (c071a7c0): kobject_add_internal: parent: 'devices', set: 'devices'
	...
	error = create_dir(kobj);  /*根据kobj->name，以及父亲的对象路径，在sys中建立目录*/
								// 如果没有父对象，则在&sysfs_root; 建立
	if (error) {
...
void kset_init(struct kset *k)
{
	...
	INIT_LIST_HEAD(&k->list);     // 这里建立了链表头
				...
				struct kobject *parent_kobj)
{
	...
	if (!kset)
		return NULL;
	retval = kobject_set_name(&kset->kobj, name); /*设置kobj->name*/
	if (retval) {
		kfree(kset);
		return NULL;
	}
				 ...
				 struct kobject *parent_kobj)
{
	...
	kset = kset_create(name, uevent_ops, parent_kobj);  // 分配并设置这个结构体，设定父对象
	if (!kset)
		return NULL;
