...
// 用probe_index 来表示第几个port，这个函数应该会被调用4次
...
static int __init s3c24xx_serial_modinit(void)
{
	...
	ret = uart_register_driver(&s3c24xx_uart_drv);  // 把驱动注册进tty层
	if (ret < 0) {
