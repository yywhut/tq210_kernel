...
uint8_t charge_ok_status = 0;
static uint8_t charge_times = 0;

static struct wake_lock oz886_wake_lock;
					...
					union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CURRENT_NOW:
		val->intval = batt_info->fCurr * 1000;
		break;
	...
	case POWER_SUPPLY_PROP_TEMP:
		val->intval = batt_info->fCellTemp * 10;
		break;

	case POWER_SUPPLY_PROP_CHARGE_FULL_DESIGN:
		val->intval = gas_gauge->fcc_data;
		break;

	case POWER_SUPPLY_PROP_CHARGE_NOW:
		val->intval = batt_info->sCaMAH;
		break;
...
static void oz8806_battery_work(struct work_struct *work)
{
	...
	*/
	...
	if((batt_info->fRSOC <= 0)&& (gas_gauge->vbus_ok)&& (batt_info->fCurr > 0))
		batt_info->fRSOC = 1;
			 ...
			 const struct i2c_device_id *id)
{
	...
	wake_lock_init(&oz886_wake_lock, WAKE_LOCK_SUSPEND, "oz8806_wake");
	wake_lock(&oz886_wake_lock);
