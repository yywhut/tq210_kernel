...
devices_init
/*
首先其中用到了devices_kset对象，这个对象和第3节当中的bus_kset是同样的性质，
也就是说该对象表示一个目录。
该对象的建立是在devices_init函数中完成的。
*/
void device_initialize(struct device *dev)
{
	//kobj，kset结构体中包含有一个kobj，一个kobj生成一个目录，
	dev->kobj.kset = devices_kset;   /*设置kobj属于哪个kset，/sys/devices/*/ // 也就是说会放在 /sys/devices 这个目录 
										// 也就是说该dev.kobj添加到了devices_kset容器中。
	kobject_init(&dev->kobj, &device_ktype);  /*初始化dev->kobj*/ // 就是初始化链表头之类，把type给 kobj
	INIT_LIST_HEAD(&dev->dma_pools);
	mutex_init(&dev->mutex);    /*初始化互斥体*/  
	lockdep_set_novalidate_class(&dev->mutex);  /*初始化自旋锁*/  
	spin_lock_init(&dev->devres_lock);
	INIT_LIST_HEAD(&dev->devres_head);
	device_pm_init(dev);      /*设置该device可操作,电源管理相关*/ 
 ...
 */
/*
这里在建立 /sys/devices/platform 这个device 跟建立
/sys/devices/platform/spi  这个devices 是不同的
首先，在device_add被调用之前，有若干个非常重要的条件已经被设置了。如下：
pdev->dev->kobj.kset = devices_kset
pdev->dev-.parent = &platform_bus
pdev->dev.bus = &platform_bus_type
set_up函数执行时，由于参数parent为&platform_bus，
因此最后将设置pdev->dev->kobj.parent = platform_bus.kobj。
平台设备对象的父对象为虚拟的platform设备。
*/
...
int device_add(struct device *dev)
{
	...
	parent = get_device(dev->parent);   　// 增加对dev->parent的引用
										// 当执行具体的 devices的时候就有父对象了
										// 当执行RTC的时候，这里是有父对象的 pdev->dev.parent = &platform_bus;


/* 设置kobj.parent，由于 s3c_device_rtc的dev不存在class，所以 dev->kobj.parent =
dev->parent.kobj，也就是等于platform_bus.kobj ，后面会详细分析
*/
	setup_parent(dev, parent);  // 如果是rtc的话，在这里dev->kobj.parent就有值了
	...
	// 第一次 platform走的时候也就是在 /sys/devices/ 下建立 platform这个设备


	//如果是 platform调用过来的则在调用时，参数parent为NULL，且dev->kobj.kset在6.1节device_initialize函数中设置为devices_kset。
...
//但是这里比较奇怪的是，为什么platform目录没有对应的kset对象？？？
	...
	/*主设备号不为0*/  // 在建立platform/ devisces 的时候这里是空的
	// 如果是rtc，这里应该是没有创建的
	if (MAJOR(dev->devt)) {   
	...
	// 在将platform这个devices 注册到 /sys/devices/platform的时候，bus是空的
	// 但是在之后实际注册设备的时候，这个地方肯定是有的
	//由于dev->bus未指定，因此这个函数什么都没干。
	// 如果是spi注册的时候，就在这里向 /sys/bus/platform/devices/下面增加链接
	// 其实就是向总线 注册这个devices，都是连接链接过来。
	...
	*/
	...
	bus_probe_device(dev);    /*将设备添加到总线的设备链表中，并尝试获取驱动*/
	if (parent)
...
{
	device_initialize(dev);   /*初始化dev的某些字段*/ // 一调用这个肯定就在/sys/devices目录下了
								// 因为这里将kset设定为devices_kset了，所以为后文 platform 注册提供了地址
