...
int platform_device_register(struct platform_device *pdev)
{
	...
	////dev->kobj.kset = devices_kset;   /*设置kobj属于哪个kset，
	//这一步最重要的就是这里，为以后目录结构做准备。
	// 剩下的都是初始化kobj结构体
...
/*
driver_register函数就是driver注册的核心函数。
需要注意的是，在调用函数之前，
...
struct bus_type platform_bus_type = {
	...
	.dev_attrs	= platform_dev_attrs,  /* 该属性文件将产生在所有 platform_bus_type类型的设备目录下，文件名为"modalias” */
	.match		= platform_match,   // 用于drive与device匹配的例程
	.uevent		= platform_uevent,  // 用于输出环境变量，与属性文件“uevent”相关
	.pm		= &platform_dev_pm_ops,  // 电源管理方面
};
EXPORT_SYMBOL_GPL(platform_bus_type);


// 初始化 platform bus
// 这里是建立总的 platform bus，同时在devices下面也建立了 plafform 这个device
int __init platform_bus_init(void)
{
	...
	error = device_register(&platform_bus);   //会建立一个 /sys/devices/platform
	if (error)
		return error;
