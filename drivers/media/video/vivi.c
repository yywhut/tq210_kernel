...
struct vivi_dev {
	...
	struct v4l2_ctrl_handler   ctrl_handler; // 这个是用来管理属性的

	/* controls */
	struct v4l2_ctrl	   *brightness;   // 这些是属性值
...
static int __init vivi_create_instance(int inst)
{
	...
	// 其实这个东西只是做了一系列的初始化，并没有注册什么东西
	ret = v4l2_device_register(NULL, &dev->v4l2_dev);// 初始化一些自旋锁，引用计数等
	if (ret)
		goto free_dev;
	...
	// 这里是设置属性，这个摄像头一共支持多少属性，这里就设置多少，然后将这些链表放入链表串。
	// 那么 vfd->ctrl_handler 在哪里设置，在video_register_device 中设置
	...
	*vfd = vivi_template;
	...
	// 把vivi_dev 下面的 v4l2_device 付给video_device 
	// 这句话还挺重要
	...
	//把vivi_dev 付给了 video_device 的私有
