...
* 其会不停增长，增长速度与load权重成反比，load越高，增长速度越慢，就越可能处于红黑树最
左边被调度,每次时钟中断都会修改其值,具体见calc_delta_fair()函数
*/
	u64			vruntime;
...
/* task state */
	...
	pid_t pid;/* 进程ID，每个进程(线程)的PID都不同 */
	 /* 线程组ID，同一个线程组拥有相同的pid，与领头线程(该组中第一个轻量级进程)pid一致，
	 保存在tgid中，线程组领头线程的pid和tgid相同 */
	pid_t tgid;
...
/* 在进程切换时保存硬件上下文(硬件上下文一共保存在2个地方: 
thread_struct(保存大部分CPU寄存器值，包括内核态堆栈栈顶地址和IO许可权限位)，
内核栈(保存eax,ebx,ecx,edx等通用寄存器值)) */
...
/*
同理，此函数可以算是 "为整个thread_info结构复制",这样子进程的thread_info 和父进程的
thread_info 相同。个问题，thread_info里面有一个指向task_struct的指针 ，子进程指向子进
程的，父进程指向父进程的，而现在，这两个thread_info中的某个指针，都指向了父进程的
task_struct，所以还要使得子进程thread_info的指针指向tsk的task_struct，而不是 
父进程的task_struct：task_thread_info(p)->task = p;
*/
...
/*
通常使用内存是从低向高地址用，但栈是预留的空间，栈底在高地址，push时，
数据是向低地址存放。最好还是看看书，一定要弄明白，因为局部变量寻址，要用到栈的访问。
