...
/* ideal_runtime为进程应该运行的时间
 ...
 /* 如果cfs_rq队列的last，next，skip指针中的某个等于当前进程，则清空cfs_rq队列中的相应指针 */
	...
	if (cfs_rq->nr_running > 1) {
		struct sched_entity *se = __pick_first_entity(cfs_rq); /* 获取下一个调度进程的se */
		s64 delta = curr->vruntime - se->vruntime;   /* 当前进程的虚拟运行时间 - 下个进程的虚拟运行时间 */

		if (delta < 0)  /* 当前进程的虚拟运行时间 大于 下个进程的虚拟运行时间，说明这个进程还可以继续运行 */
			return;

 /* 当前进程的虚拟运行时间 小于 下个进程的虚拟运行时间，说明下个进程比当前进程更
 应该被CPU使用，resched_curr()函数用于标记当前进程需要被调度出去 */
		if (delta > ideal_runtime)
			resched_task(rq_of(cfs_rq)->curr);/* 标记当前进程需要调度，将当前进程的thread_info->flags设置TIF_NEED_RESCHED标
	}
}
