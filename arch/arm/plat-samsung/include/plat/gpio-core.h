 ...
 */
struct s3c_gpio_chip {
	struct gpio_chip	chip;  // 这里里面包含组的base的numer，所有的gpio从0开始排列的那个
	struct s3c_gpio_cfg	*config;
	struct s3c_gpio_pm	*pm;
	void __iomem		*base;    // 这个gpio组的虚拟地址，怎么得到某一个gpio的虚拟地址
	int			irq_base;
	int			group;      // 表示是哪个组，从0开始
	spinlock_t		 lock;
#ifdef CONFIG_PM
	u32			pm_save[4];
#endif
