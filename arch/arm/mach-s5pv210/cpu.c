...
{
	iotable_init(s5pv210_iodesc, ARRAY_SIZE(s5pv210_iodesc));   //这是为SOC硬件寄存器创建内存映射
...
//xtal  是24000000
	s3c24xx_register_baseclocks(xtal);  // 挂了一堆到连表上
	s5p_register_clocks(xtal);//一堆频率加到链表上
	s5pv210_register_clocks();
	s5pv210_setup_clocks();  // 计算了 f h c 并注册
}
